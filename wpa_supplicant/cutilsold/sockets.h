...
#define ANDROID_SOCKET_DIR		"/data/data/as.wps.wpatester/socket"
...
static inline int android_get_control_socket(const char* name)
{
	...
	printf("SONO ENTRATO SU GETCONTROL SOCKET\n");
	const char* val = getenv(key);
	if (!val) {
		printf("VALORE HA TORNATO -1\n");
		return 4;
	}
	...
	if (errno) {
		printf("NON RIESCO A PRENDERE IL FILE DESCRIPTOR\n");
		return -1;
	}
